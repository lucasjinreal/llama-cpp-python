...
specified) expect poor results""",
        ...
        if self.params.random_prompt:
            self.params.prompt = 'hello, how are u?'
        ...
        if not self.ctx:
            raise RuntimeError(
                f"error: failed to load model '{self.params.model}'")
...
| {llama_cpp.llama_print_system_info().decode('utf8')}",
        ...
        if self.params.mem_test:
            print('mem test should not used in production!')
            tmp = [0, 1, 2, 3]
        ...
        if len(self.params.path_session) > 0:
            ...
            if path.exists(self.params.path_session):
                _session_tokens = (llama_cpp.llama_token *
                                   (self.params.n_ctx))()
        ...
        self.inp_suffix = self._tokenize(
            self.params.instruct_inp_suffix, False)
        ...
        if self.params.instruct:
            ...
            _ptn = self._tokenize(
                self.params.instruct_inp_prefix.strip(), False)
...
number of tokens in prompt = {len(self.embd_inp)}""",
            ...
            for i in range(len(self.embd_inp)):
                print(
                    f"{self.embd_inp[i]} -> '{llama_cpp.llama_token_to_str(self.ctx, self.embd_inp[i])}'",
                    file=sys.stderr,
                )
        ...
        if self.params.interactive:
            ...
            if len(self.params.input_prefix) > 0:
                print(
                    f"Input prefix: '{self.params.input_prefix}'", file=sys.stderr)
...
""",
        ...
        # TODO: deque doesnt support slices
        self.last_n_tokens = [0] * self.n_ctx
...
""",
    ...
    def _tokenize(self, prompt, bos=True):
        ...
        _n = llama_cpp.llama_tokenize(
            self.ctx, prompt.encode(
                "utf8", errors="ignore"), _arr, len(_arr), bos
        )
        return _arr[:_n]
    ...
    def generate(self):
        while (
            ...
            if len(self.embd_inp) <= self.input_consumed:  # && !is_interacting
                ...
                last_n_repeat = min(len(self.last_n_tokens),
                                    repeat_last_n, self.n_ctx)
                ...
                llama_cpp.llama_sample_repetition_penalty(
                    ...
                    _arr,
                    last_n_repeat,
                    llama_cpp.c_float(self.params.repeat_penalty),
                )
                llama_cpp.llama_sample_frequency_and_presence_penalties(
                    ...
                    _arr,
                    last_n_repeat,
                    llama_cpp.c_float(self.params.frequency_penalty),
                    llama_cpp.c_float(self.params.presence_penalty),
                )
                ...
                if self.params.temp <= 0:
                    ...
                    id = llama_cpp.llama_sample_token_greedy(
                        self.ctx, candidates_p)
                else:
                    if self.params.mirostat == 1:
                        mirostat_mu = 2.0 * self.params.mirostat_tau
                        mirostat_m = 100
                        llama_cpp.llama_sample_temperature(
                            self.ctx, candidates_p, llama_cpp.c_float(
                                self.params.temp)
                        )
                        ...
                        llama_cpp.llama_sample_top_k(
                            self.ctx, candidates_p, top_k, min_keep=self.params.n_keep
                        )
                        llama_cpp.llama_sample_tail_free(
                            self.ctx,
                            candidates_p,
                            llama_cpp.c_float(self.params.tfs_z),
                            min_keep=self.params.n_keep,
                        )
                ...
                while len(self.embd_inp) > self.input_consumed:
                    ...
                    self.last_n_tokens.pop(0)
                    self.last_n_tokens.append(
                        self.embd_inp[self.input_consumed])
    ...
    def past(self):
        for id in self.last_n_tokens[-self.n_past:]:
            yield llama_cpp.llama_token_to_str(self.ctx, id).decode(
                "utf8", errors="ignore"
            )
